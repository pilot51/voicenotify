		...
		when (intent.action!!) {
			ACTION_TOGGLE -> if (isRunning) {
				Toast.makeText(context,
					if (toggleSuspend()) R.string.service_suspended else R.string.service_running,
			...
			ACTION_ON -> if (isRunning) {
				Toast.makeText(context,
					if (toggleSuspend(false)) R.string.service_suspended else R.string.service_running,
					Toast.LENGTH_SHORT).show()
			}
			ACTION_OFF -> {
				if (isRunning) {
					Toast.makeText(context,
						if (toggleSuspend(true)) R.string.service_suspended else R.string.service_running,
						Toast.LENGTH_SHORT).show()
				}
		...
		private fun updateViews(context: Context, views: RemoteViews) {
			...
			if (isRunning) {
				pendingIntent = PendingIntent.getBroadcast(context, 0,
					Intent(context, WidgetProvider::class.java).setAction(ACTION_TOGGLE), 0)
				if (isSuspended) {
				...
				pendingIntent = PendingIntent.getActivity(context, 0, notificationListenerSettingsIntent, 0)
